...
class RepPointsHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        #                                               self.dcn_kernel, 1,
        #                                               self.dcn_pad)
        # self.reppoints_pts_refine_conv = nn.Conv2d(self.feat_channels,
        #                                               self.point_feat_channels,
        #                                            3,1,1)
    ...
    def stars_to_offset(self, predict_stars, gt_bboxes):
        ...
        pre_h = predict_stars[:, 3]
        pre_yc_col = pre_yc_row
        ...
        return delta_bboxes
    def offset_to_stars(self, star_init, pred_offset,img_metas):
        """Change from star offset to star coordinate."""
        ...
        init_w = star_init[..., 2]
        init_x_col = init_x_row
        init_y_col = init_y_row
        init_h = star_init[..., 3]
    ...
    def forward_single(self, x):
        ...
        cls_out = self.reppoints_cls_out(
            self.relu(self.reppoints_cls_conv(cls_feat)))
        # pts_out_refine = self.reppoints_pts_refine_out(
        ...
        return cls_out, pts_out_init,bbox_out_init#, pts_out_refine
    ...
    def _point_target_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            pos_gt_bboxes = self.stars_to_offset(proposals[pos_inds, :],
                                                 pos_gt_bboxes)
    ...
    def loss_single(self, cls_score, pts_pred_init, #pts_pred_refine,
        ...
        bbox_pred_init=pts_pred_init.permute(0, 2, 3,
                                      1).reshape(-1,4)
    ...
    def loss(self,
        ...
        (labels_list, label_weights_list, bbox_gt_list_init, candidate_list_init, bbox_weights_list_init,
         num_total_pos_init, num_total_neg_init) = cls_reg_targets_init
        num_total_samples_init = (
            num_total_pos_init +
        ...
        losses_cls, losses_pts_init = multi_apply(
            self.loss_single,
            cls_scores,
            pts_preds_init,
    ...
    def get_bboxes(self,
                   ...
                   rescale=False,
                   with_nms=True):
        assert len(cls_scores) == len(pts_preds_init)
        device = cls_scores[0].device
        bbox_preds_init = [
            self.star2bbox([], pts_pred_init.detach(), img_metas[0], mode='xywxyhtox1y1x2y2')
            # self.star2bbox(pts_pred_init)
            for pts_pred_init in pts_preds_init
        ]
        # for i in range(0,len(pts_preds_init)):
        #     reg_out_init_star = self.offset_to_stars(torch.stack(stars_init, dim=0),
        #                                              reg_out_init.permute(0, 2, 3, 1).detach(), img_metas)  # stars
